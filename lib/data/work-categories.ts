...
export const getWorkCategoriesByDepartmentFlag = (departmentFlag: number): WorkCategory[] => {
  return workCategories.filter(
    category => category.department_flag === departmentFlag && category.status === 'active'
  );
};