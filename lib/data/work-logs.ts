...
export function getWorkLogWithDetails(workLogId: number) {
  ...
  const workCategory = workLog.work_category_id ? getWorkCategoryById(workLog.work_category_id) : null;
  const workSubCategory = workLog.work_sub_category_id ? getWorkSubCategoryById(workLog.work_sub_category_id) : null;
  const project = workLog.project_id ? getProjectById(workLog.project_id) : null;
...
export function addWorkLog(workLog: Omit<WorkLog, 'id' | 'created_at' | 'updated_at'>): WorkLog {
...
export function updateWorkLog(id: number, workLog: Partial<WorkLog>): WorkLog | null {
...
export function filterWorkLogs(filters: {
  ...
  return workLogs.filter(log => {
    ...
    if (filters.departmentId !== undefined && log.department_id !== filters.departmentId) return false;
    if (filters.userId !== undefined && log.user_id !== filters.userId) return false;
    if (filters.customerId !== undefined && log.customer_id !== filters.customerId) return false;
    if (filters.projectId !== undefined && log.project_id !== filters.projectId) return false;
    if (filters.workCategoryId !== undefined && log.work_category_id !== filters.workCategoryId) return false;
    if (filters.workStatus !== undefined && log.work_status !== filters.workStatus) return false;
